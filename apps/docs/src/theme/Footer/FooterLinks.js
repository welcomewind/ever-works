...
export function FooterLinks({ columns }) {
    return (<div className="flex flex-wrap gap-6 justify-center lg:justify-start max-w-[70%] grow">
			{columns.map((column, index) => (<div key={index} className="flex flex-col gap-3 min-w-fit basis-36 grow w-fit">
                ...
                return (<li key={itemIndex}>
									<Link to={url} target={isExternal ? '_blank' : undefined} rel={isExternal ? 'noopener noreferrer' : undefined} className="hover:underline hover:underline-offset-2">
										<span className="inline-flex gap-x-2 items-center text-sm text-gray-600 capitalize group hover:text-gray-800 dark:text-gray-400 dark:hover:text-white">
											...
											{isExternal && (<svg className="align-baseline transition-transform duration-300 ease-in-out size-4 group-hover:translate-x-1 group-hover:rotate-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 18 18" fill="currentColor">
