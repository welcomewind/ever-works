...
// Icons
const SearchIcon = () => (<svg className="w-4 h-4 text-[#808098]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
		<path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/>
	</svg>);
const CloseIcon = () => (<svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
...
export default function SearchBar() {
    ...
    const loadIndex = useCallback(async () => {
        if (indexLoaded || indexLoading || !isBrowser)
            return;
        try {
            setIndexLoading(true);
            setError(null);
            // Dynamically import the search worker functions
            const { fetchIndexesByWorker } = await import('@easyops-cn/docusaurus-search-local/dist/client/client/theme/searchByWorker');
    ...
    const performSearch = useCallback(async (searchQuery) => {
        ...
        try {
            setIsLoading(true);
            setError(null);
            const { searchByWorker } = (await import('@easyops-cn/docusaurus-search-local/dist/client/client/theme/searchByWorker'));
            ...
            const searchResults = await searchByWorker(searchBaseUrl, '', searchQuery, 8 // searchResultLimits - max number of search results
            );
    ...
    return (<>
			...
			{/* Search Modal */}
			{isOpen && (<div className="fixed inset-0 z-[9999] flex items-start justify-center pt-[10vh] px-4 animate-fade-in">
					{/* Backdrop */}
					<div className="absolute inset-0 bg-black/50 dark:bg-black/70 backdrop-blur-sm"/>

					{/* Modal */}
					<div ref={modalRef} className="relative w-full max-w-2xl bg-white dark:bg-black rounded-2xl shadow-2xl border border-gray-200 dark:border-gray-700 overflow-hidden animate-slide-up" onKeyDown={handleKeyDown}>
						{/* Search Input */}
						<div className="flex items-center gap-3 px-4 py-3 border-b border-gray-200 dark:border-gray-700">
							<SearchIcon />
							<input ref={inputRef} type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search documentation..." className="flex-1 bg-transparent text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 outline-none text-base" autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck="false"/>
							{query && (<button onClick={() => setQuery('')} className="p-1 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
									<CloseIcon />
								</button>)}
							<button onClick={closeModal} className="px-2 py-1 text-xs font-medium text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-700 rounded border border-gray-200 dark:border-gray-600 hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">
								ESC
							</button>
						</div>
						...
						<div className="max-h-[60vh] overflow-y-auto">
							...
							{error && (<div className="flex flex-col items-center justify-center py-12 text-red-500 dark:text-red-400">
							...
							{!error && (isLoading || indexLoading) && (<div className="flex flex-col items-center justify-center py-8">
							...
							{/* No results found */}
							{!error && !isLoading && !indexLoading && results.length === 0 && query && indexLoaded && (<div className="flex flex-col items-center justify-center py-12 text-gray-500 dark:text-gray-400">
									<SearchIcon />
									<p className="mt-4 text-sm">No results found for &quot;{query}&quot;</p>
									{process.env.NODE_ENV !== 'production' && (<div className="mt-4 px-4 py-2 bg-amber-100 dark:bg-amber-900/30 border border-amber-300 dark:border-amber-700 rounded-lg text-amber-800 dark:text-amber-200 text-xs text-center max-w-sm">
							...
							{!error && !isLoading && !indexLoading && results.length === 0 && !query && (<div className="flex flex-col items-center justify-center py-12 text-gray-500 dark:text-gray-400">
							...
							{!isLoading && results.length > 0 && (<ul className="p-2">
									{results.map((result, index) => (<li key={`${result.document.u}-${index}`}>
											<button onClick={() => navigateToResult(result)} className={`w-full flex items-start gap-3 p-3 rounded-lg text-left ${index === selectedIndex
                        ...
                        : 'hover:bg-gray-100 dark:hover:bg-gray-800/50'}`} onMouseEnter={() => setSelectedIndex(index)}>
												<div className={`flex-shrink-0 p-1.5 rounded-lg ${index === selectedIndex
                        ? 'bg-purple-500/20 text-purple-600 dark:text-purple-400'
                        : 'bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400'}`}>
													<DocIcon />
												</div>
												<div className="flex-1 min-w-0">
													<div className={`font-medium truncate ${index === selectedIndex
                        ? 'text-purple-700 dark:text-purple-300'
                        : 'text-gray-900 dark:text-white'}`}>
														{result.document.t}
													</div>
													{result.document.s && (<div className="text-sm text-gray-500 dark:text-gray-400 truncate mt-0.5">
															{result.document.s}
														</div>)}
												</div>
												{index === selectedIndex && (<div className="flex-shrink-0 text-purple-500 dark:text-purple-400">
